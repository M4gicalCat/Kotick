...
export default {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      'Marque un événement comme terminé et le met à jour depuis une google sheet',
    )
    .addStringOption(o =>
      o
        .setName('event_link')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const names = await db.any(
      `SELECT id, name FROM discord.user_name WHERE name IN ($1:csv) AND guild_id = $2`,
      [[...new Set(actis.flatMap(a => a.responsables))], interaction.guildId],
    );
    ...
    await db.none(
      `UPDATE discord.event SET meeting_done = true WHERE event_id = $1 AND guild_id = $2`,
      [googleEvent, interaction.guildId],
    );
